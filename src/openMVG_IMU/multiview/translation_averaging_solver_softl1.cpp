...
bool solve_priotranslations_problem_softl1
(
...
)
{
  ...
  std::cout<<"nb_poses==prio_translation.size()="<<nb_poses<<"\n";
  for (unsigned int i = 0; i < nb_poses; ++i, mycpt+=3)
  {
    vec_translations[mycpt]=prio_translation[i](0);
    vec_translations[mycpt+1]=prio_translation[i](1);
    vec_translations[mycpt+2]=prio_translation[i](2);
  }
  mycpt = 0;
