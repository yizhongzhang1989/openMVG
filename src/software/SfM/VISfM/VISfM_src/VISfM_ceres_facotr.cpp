...
namespace openMVG
{
    namespace sfm
    {
        ...
        Eigen::Matrix2d VISfM_ProjectionSim3::sqrt_info = Eigen::Matrix2d::Identity();
...
//                    std::cout << "========================" << std::endl;
        ...
        VISfM_ProjectionSim3::VISfM_ProjectionSim3(Eigen::Vector2d &obs) : point_obs_(obs)
        {

        }

        bool VISfM_ProjectionSim3::Evaluate(const double *const *parameters, double *residuals, double **jacobians) const
        {
            Eigen::Map<const Eigen::Vector3d> twi(parameters[0]);
            Eigen::Quaterniond Qwi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);
            Eigen::Map<const Eigen::Vector3d> tic(parameters[1]);
            ...
            Eigen::Vector3d pts_imu = Qwi.inverse() * (pts_w - twi*scale);
            Eigen::Vector3d pts_camera = Qic.inverse() * (pts_imu - tic);
            Eigen::Vector2d pts_c_nrom = pts_camera.head(2);
            pts_c_nrom /= pts_camera(2);
...
//            std::cout << "residuals = " << residual.transpose() << std::endl;
            ...
            if( jacobians )
            {
                ...
                if(jacobians[0])
                {
                    ...
                    Eigen::Matrix<double, 3, 6> jacobian_ptsc_pose;
                    jacobian_ptsc_pose.block<3,3>(0,0) = - Ric.transpose() * Rwi.transpose() * scale;
                    jacobian_ptsc_pose.block<3,3>(0,3) = Ric.transpose() * Utility::skewSymmetric(pts_imu);
...
//                    jacobian_ex_pose.block<2, 1>(0, 2).setZero();
                ...
                if(jacobians[4])
                {
                    Eigen::Map<Eigen::Vector2d> jacobian_scale(jacobians[4]);
                    jacobian_scale.setZero();

                    Eigen::Matrix<double, 3, 3> jacobian_ptsc_pts_imu = Ric.transpose();

                    Eigen::Matrix<double, 3, 1> jacobian_ptsc_imu_scale = Qwi.inverse() * -twi;

                    jacobian_scale  = sqrt_info * jacobian_e_ptscn * jacobian_ptscn_ptsc * jacobian_ptsc_pts_imu * jacobian_ptsc_imu_scale;
                }
                bool numb_jaco = false;
                if( numb_jaco )
                {
