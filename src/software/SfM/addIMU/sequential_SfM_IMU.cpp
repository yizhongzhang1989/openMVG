      ...
      cam_type_(EINTRINSIC(PINHOLE_CAMERA_RADIAL3))
{
    ...
    cout<<"[Inside SequentialSfMReconstructionEngineIMU Constructor]"<<endl;
    cout<<"Image Stamps:"<<endl;
    int count = 0;
    for(Views::const_iterator itV = sfm_data.GetViews().begin(); itV != sfm_data.GetViews().end(); itV++)
    {
        View* v = itV->second.get();
        IndexT idx = v->id_view;
        std::string filename = v->s_Img_path;
        cout<<filename<<" "<<view_stamps.at(idx)<<endl;
...
void SequentialSfMReconstructionEngineIMU::SetFeaturesProvider(Features_Provider * provider)
{
  features_provider_ = provider;
}

void SequentialSfMReconstructionEngineIMU::SetMatchesProvider(Matches_Provider * provider)
{
...
bool SequentialSfMReconstructionEngineIMU::ChooseInitialPair(Pair & initialPairIndex) const
{
  if (initial_pair_ != Pair(0,0))
  {
...
bool SequentialSfMReconstructionEngineIMU::InitLandmarkTracks()
{
...
bool SequentialSfMReconstructionEngineIMU::AutomaticInitialPairChoice(Pair & initial_pair) const
{
...
bool SequentialSfMReconstructionEngineIMU::MakeInitialPair3D(const Pair & current_pair)
{
...
double SequentialSfMReconstructionEngineIMU::ComputeResidualsHistogram(Histogram<double> * histo)
{
 ...
 */
bool SequentialSfMReconstructionEngineIMU::FindImagesWithPossibleResection(
 ...
 */
bool SequentialSfMReconstructionEngineIMU::Resection(const uint32_t viewIndex)
{
...
bool SequentialSfMReconstructionEngineIMU::BundleAdjustment()
{
 ...
 */
bool SequentialSfMReconstructionEngineIMU::badTrackRejector(double dPrecision, size_t count)
{
