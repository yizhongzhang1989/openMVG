	...
	const std::string& sPoints3DFilename)
{
	...
	if (!ColorizeTracks(sfm_data, vec_3dPoints, vec_tracksColor)) {
		std::cout << "create color failed, set to white color" << std::endl;
		const Vec3 white_color(255., 255., 255.);
		vec_tracksColor.clear();
		vec_tracksColor.resize(vec_3dPoints.size(), white_color);
		//return false;
	}
...
int main(int argc, char** argv)
{
	...
	std::string sOutDir = "";
	std::string sImageRoot = "";

	cmd.add(make_option('i', sSfM_Data_Filename, "sfmdata"));
	cmd.add(make_option('o', sOutDir, "outdir"));
	cmd.add(make_option('r', sImageRoot, "imageroot"));
	...
	catch (const std::string& s)
	{
		std::cerr << "Usage: " << argv[0] << '\n'
			...
			<< "[-r|--imageroot] path of images root\n"
			<< std::endl;
	...
	if (sImageRoot != "")
		sfm_data.s_root_path = sImageRoot;
