...
int main(int argc, char** argv)
{
	...
	// Optional
	cmd.add(make_option('g', gravity, "gravity"));
	...
	catch (const std::string& s)
	{
		std::cerr << "Usage: " << argv[0] << '\n'
			<< "[-p|--trajectory_file] Obj file for generating trajectory, recommended to generate from curve in blender \n"
			<< "[-m|--model_file] Obj file for generating structure (3D model), points will be sampled on the model surface \n"
